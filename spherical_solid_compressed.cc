   ...
   for(unsigned i=0;i<n_nod;i++)
    {
        ...
        //get z position
        double z = x[0] * cos(xi[1] + x[1]);

        if(z > Global_Physical_Variables::H)
        {
                //Need to set R and Theta such that it is below H
                //Move boundary radially
                double newR = Global_Physical_Variables::H / (cos(xi[1] + x[1]));
                nod_pt->x(0) = newR;
        }
    }
