...
//Added by Edgar 28/04/2016

//======================================================================
/// \short Penetrator -- for Axis-Symmetric solid
//======================================================================
class AxiSymPenetrator : public virtual Penetrator
 {

   public:

  /// Constructor: Pass reference to height in z-direction
  AxiSymPenetrator(double* Height) {H=Height;}

  /// Destructor
  virtual ~AxiSymPenetrator(){}
  ...
  void penetration(const Vector<double>& x,
                     ...
                     bool& intersection)const
  {
   //Working in axis-symmetric basis
   // so x[0] = R and x[1] = Theta
   // in basis of where deformed angle is theta + Theta

   //Need to project onto the z-axis. Either I need the undeformed angle as well
   // or need to change code so that it passes the actual angle rather than  Theta
   // or hijack the normal n as the lagrangian coordinates
   // double xpos = x[0]*sin(n[1]) + x[1]*cos(n[1]);
    double zpos2 =  x[0]*cos(n[1]) - x[1]*sin(n[1]);

    // double zpos = x[0] * cos(n[1] + x[1]); // This didn't work, caused wrong behaviour?
//   double zpos = x[0] * cos(x[1]);
    ...
    for (unsigned j=0;j<nplot;j++)
     {
      ...
      outfile << double(j)/double(nplot-1) << " " 
              << *H
              << std::endl;
     }
   }
...
//       Add by Edgar on 28/04/2016 to work with Axisymmetric Spherical Solid
//      elements in r, phi direction instead of a carthesian basis
...
/// permament contact (in which case contact force can be
/// positive or negative. Uses Lagrange-multiplier-like pressure
/// to enforce contact/non-penetration. Works only with
/// 2D Penetrator at the moment. Specific implementation for
/// nonlinear elasticity in the case of axisymmetric solid.
//======================================================================
template <class ELEMENT>
class AxiSymNonlinearSurfaceContactElement: 
public virtual SurfaceContactElementBase<ELEMENT>
{

 private:
 ///Holds the force excerted on this element, needs to be calculated 
 /// in the residual by integrating the pressure
  Vector<double> force; 
 ...
 /// \short Constructor, which takes a "bulk" element and the 
 /// value of the index and its limit
  AxiSymNonlinearSurfaceContactElement(FiniteElement* const &element_pt, 
                                 const int &face_index,
                                 const unsigned &id=0,
                                 const bool& called_from_refineable_constructor=
                                 false) : 
...
//    this->outer_unit_normal(s,unit_normal);   
    ...
    double d=0.0;
    bool intersection = false;
    //Need lagrangian coordinates as well, pass instead of unit normal
    //this->penetration(x,unit_normal,d,intersection);
    this->penetration(x,xi,d,intersection);
...
//     double theta = xi[1];
       ...
       //Also output undeformed and deformed coordinates
    outfile << xi[0] << " " << xi[1] << " "
	    << x[0] << " " << x[1] << " ";
...
//     for(unsigned i=0;i<n_dim;i++) 
//      {outfile << x[i]-xi[i] << " ";}
    //ToDo: output contact area and force
    Vector<double> force(2);
    this->resulting_contact_force(force);
    outfile <<  force[0] << " "; 
    outfile <<  force[1] << " "; 

    //Decompose the force in x and z direction
    double f_x = force[0] * sin(xi[1])   + force[1] * cos(xi[1]);
    double f_z = force[0] * cos(xi[1])   - force[1] * sin(xi[1]);
  ...
  /// \short Output function that outputs at integration points
 void output_nodal(std::ostream &outfile)
 {
  unsigned n_dim = this->nodal_dimension();
  ...
  for(unsigned l=0;l<n_node;l++) 
    ...
    SolidNode *solid_nod_pt = 
      dynamic_cast<SolidNode*>(this->node_pt(l));
    
       
    // Loop over directions to calculate the position
    for(unsigned j=0;j<n_dim;j++)
      {
        x[j] =solid_nod_pt->x(j);
        xi[j] =solid_nod_pt->xi(j);
      }
    ...
    // Pressure (Lagrange multiplier) is the first (and only) additional
    // value created by this face element
    double p =this->node_pt(l)->value(first_index);
 ...
 /// \short Output function that outputs at integration points
 void output_int(std::ostream &outfile)
 {
   unsigned n_dim = this->nodal_dimension();
...
//        s[i]=this->integral_pt()->knot(ipt,i);
//       }   
     s[0] = this->integral_pt()->knot(ipt,0); //at least for the my integration schema, this should be the thing
     
     // Get Eulerian and Lagrangian coordinatesthis->node_pt(l) 
     this->interpolated_x(s,x);
     this->interpolated_xi(s,xi);     
...
//=====================================================================
template<class ELEMENT>
void AxiSymNonlinearSurfaceContactElement<ELEMENT>::
fill_in_contribution_to_residuals_surface_contact(Vector<double> &residuals)
{
 ...
 Vector<double> local_residuals(n_dof,0.0);
 {
  ...
  for(unsigned ipt=0;ipt<n_intpt;ipt++)
   ...
   //Added by Edgar
   //I will mash together the two different formulations for now in a horrible 
   // mess and come back to this should this ever be submitted. Basically, its 
   // a combination of the original of the axis-symmetric implementation written 
   // by Andrew way back when and the modern way of doing it in oomph lib
   
//Calculate the global position and lagrangian coordinate
   //Vector<double> interpolated_x2(2,0.0); 
      ...
      for(unsigned k=0;k<n_position_type;k++)
       {
        ...
        for(unsigned i=0;i<n_dim;i++)
          ...
          *dpsids(l,k,0);
          ...
          //TODO: note the duplication, eliminate one or the other            
          //Loop over LOCAL derivative directions, to calculate the tangent(s)
          for(unsigned j=0;j<n_dim-1;j++)
           {
            interpolated_A(j,i) += 
...
/*
  ...
  */
   ...
   //TODO: get modern way to work
   //Old way of finding outward normal 
          ...
          if(local_eqn >= 0)
           {
            ...
            //A possibel reason for the problems in the Newton iterations is that
            // the contact pressurs are of the order 1e4 and the penetration of 1e-2
            // By pre-multiplying this, hopefully it shoudl get better
//             local_residuals[local_eqn] -= 1e8 * traction[i]*psi(l,k)*W;
           }
         }
       } //End of if not boundary condition
     } //End of loop over shape functions
//faire!!!!
    ...
    if(!this->use_collocated_penetration_flag())
     {
      ...
      bool intersection = false;
      //this->penetration(interpolated_x,interpolated_normal,d,intersection);
      //Need lagrangian coordinates so pass them instead of normal
      this->penetration(interpolated_x,interpolated_xi,d,intersection);
      if(!intersection)
       {
        //if there is no intersection then we set d as -infinity
        d = -DBL_MAX;
	...
	// Need area that is in contact only
	area += W;
      }
  ...
  for(unsigned l=0;l<n_node;l++)
   {
    // get the node pt
    //Node* nod_pt = this->node_pt(l);
    SolidNode* nod_pt =  dynamic_cast<SolidNode*>(this->node_pt(l));
        ...
        if(this->use_collocated_penetration_flag())
         {
          // Get penetration
          bool intersection = false;
        //pass lagrangian position instead of unite normal          
        this->penetration(x,xi,d,intersection);
...
//=====================================================================
template<class ELEMENT>
void AxiSymNonlinearSurfaceContactElement<ELEMENT>::resulting_contact_force(
 Vector<double> &contact_force) 
{
 //Find out how many nodes there are
 unsigned n_node = this->nnode();
...
//       if(contact_force[i] <0){
//         std::cout << "Contact force["<<i<<"] is negative = " << contact_force[i] << std::endl;
//       }
    }
