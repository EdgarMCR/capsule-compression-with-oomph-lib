  ...
  for(unsigned e=0;e<n_element;e++)
    {
      ...
      // Check flag whether to use hard or soft contact
      if (Global_Physical_Variables::hard_contact)
	{
	  ...
	  if (CommandLineArgs::command_line_flag_has_been_set("--nintpt"))
	    {
	      if(!outputed)
		{
		  cout << "setting integration schema with " 
	...
	{
	  // Build the corresponding contact element
	  AxisymmetricSolidTractionSoftContactElement<ELEMENT>* contact_element_pt = new 
	    AxisymmetricSolidTractionSoftContactElement<ELEMENT>(bulk_elem_pt,
								 face_index, 
